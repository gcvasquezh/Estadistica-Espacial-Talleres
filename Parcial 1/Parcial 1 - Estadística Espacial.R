...
sigma2 <- 3
phi <- 2
covarianza<-sigma2*exp((-distancia)/phi) # Modelo de covarianza exponencial
...
sigmasq_est=modelo_residuales$cov.pars[1]
...
datosncte.kc<-krige.conv(datosncte, loc=datosncte.grid,  krige= krige.control(nugget=0,trend.d="1st", trend.l="1st",
                        cov.pars=c(sigmasq=sigmasq_est, phi_est= phi_est)))
